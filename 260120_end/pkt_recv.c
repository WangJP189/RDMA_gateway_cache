...
// extern char g_dst_gbn_interface_name[IF_NAMESIZE]; //
// 全局目的网关GBN重传接口名称 extern char
// g_src_nack_interface_name[IF_NAMESIZE];
// // 全局源网关发送NACK接口名称 extern char
// g_src_sr_interface_name[IF_NAMESIZE];
// // 全局源网关发送SR数据接口名称
...
int start_packet_receiver(const char *interface_name) {
    ...
    if (pthread_create(&g_receiver_thread, NULL, packet_receiver_thread,
...
int is_receiver_running() { return g_receiver_running; }
...
void process_ack_packet(const unsigned char *buffer, ssize_t length,
    ...
    // 对于RC（Reliable
    // Connected）服务类型，ACK报文通常都包含AETH头，但存在一些特殊情况。
...
// void process_nack_packet(const unsigned char *buffer, ssize_t length, int
// bth_offset,
//                         struct connection_bucket *bucket, struct
//                         connection_key key, uint32_t epsn) {
//     // printf("NACK: %s:%d -> %s:%d\n", src_ip, src_port, dst_ip, dst_port);
//     // printf("PKey=0x%04x, QP=%u, PSN=%u, ePSN=%u, Syndrome=0x%02x\n",
//     //        pkey, dest_qp, psn, epsn, syndrome);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (is_packet_lost(conn, current_psn)) {
            ...
            if (seg_count >= MAX_LOST_SEGMENTS) {
                // 当seg_count ==
                // MAX_LOST_SEGMENTS时，表示这是第MAX_LOST_SEGMENTS+1个丢包段，超出限制，不做记录，退出循环
...
void send_sr_request_to_src_gateway(struct connection_key key,
    ...
    gateway_control_msg *gw_control_msg =
        (gateway_control_msg *)malloc(msg_size);
    ...
    for (uint32_t i = 0; i < seg_num; i++) {
        printf("丢包段 %d: start_psn=%d, num=%d\n", i,
               gw_data_msg->lost_segments[i].start_psn,
               gw_data_msg->lost_segments[i].loss_num);
    }
...
void *sr_retransmit_worker(void *arg) {
    ...
    // 设置SO_REUSEADDR选项
    int opt = 1;
    if (setsockopt(g_tcp_server_sockfd, SOL_SOCKET, SO_REUSEADDR, &opt,
                   sizeof(opt)) == -1) {
        perror("setsockopt失败");
    ...
    if (bind(g_tcp_server_sockfd, (struct sockaddr *)&server_addr,
...
// ZPY
int is_retransmit_running() { return g_retransmit_running; }
...
void handle_nack_received(const unsigned char *buffer, ssize_t length,
    ...
    if (cache_array != NULL) {
        ...
        printf("[INFO] handle_ack_received: "
               "匹配到目标连接，开始清理ACK确认报文 | NACK PSN=0x%06X\n",
               nack_epsn);
