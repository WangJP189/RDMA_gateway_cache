...
void *rx_thread_func(void *arg) {
    ...
    printf("RX Thread exited, total packets: %lu\n", packet_count);
    return NULL;
}
...
void *parse_thread_func(void *arg) {
    ...
    while (!gd->stop) {
        ...
        for (uint16_t i = 0; i < nb_deq; i++) {
            ...
            if (is_data_packet(bth)) {
                ...
                int ret = build_control_key_v4(&key, ip_hdr, bth, &role);
                if (ret != OK) {
                ...
                process_control_packet(gd, &key, mbuf, bth, &role);
            } else {
    ...
    printf("Parse Thread exited, total packets: %lu, errors: %lu\n",
...
uint32_t traverse_psn_with_wrap(struct cache_entry_v4 *entry,
    ...
    for (uint32_t i = 0; i < max_to_process && processed_count < max_burst;
        ...
        if (cached_psn != current_psn) {
            printf("Warning: PSN mismatch at index %u (cached:0x%06X, "
                   "expected:0x%06X)\n",
                   index, cached_psn, current_psn);
        ...
        entry->cur_psn = (current_psn + 1) & PSN_MASK; // 移动到下一个PSN
...
// 在老化线程中清理定时器
static void cleanup_entry_timer(struct cache_entry_v4 *entry,
    ...
    if (entry->rnr_timer_fd >= 0) {
        ...
        printf("Aging thread: cleaned timer for entry\n");
    }
}
...
void *aging_thread_func(void *arg) {
    ...
    while (!gd->stop) {
        ...
        uint32_t connections_removed = 0;
        uint32_t packets_removed = 0;
                ...
                if (current_time - entry->timestamp > aging_threshold) {
                    ...
                    *ppentry = entry->next;
                    ...
                    printf("Aging: removed connection with %u packets\n",
                           packets_in_entry);
                    // 继续检查当前桶的下一个条目，不需要更新ppentry
                } else {
                    // 检查连接内的报文是否过期
                    uint32_t aged_packets = 0;
                    for (int j = 0; j < MAX_PSN_ARRAY; j++) {
                        if (entry->mbuf_array[j]) {
                            struct pkt_cache *pc = entry->mbuf_array[j];
                            uint64_t packet_age = current_time - pc->recv_stamp;
                            if (packet_age > aging_threshold) {
                                destroy_pkt_cache(pc);
                                entry->mbuf_array[j] = NULL;
                    ...
                    if (aged_packets > 0) {
                        printf("Aging: removed %u old packets from active "
                               "connection\n",
                               aged_packets);
        ...
        if (current_time - last_print_time >
            (rte_get_timer_hz() * 30)) { // 30秒
            printf("Aging thread: removed %u connections, %u packets total\n",
                   connections_removed, packets_removed);
