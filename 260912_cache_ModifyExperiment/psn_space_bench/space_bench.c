...
/* ===== 档位（size-class）动态内存块机制常量（对齐 pkt_cache.h） ===== */
/* 分档环形数组：档位边界为 {256,512,1024,1536,2048,4096}；在 1024 与 2048
 * 之间增加 1536 中间档，平滑 1280~1500 B（非 2 幂次以太网帧长）下的内部碎片 dip。
 * 块大小 = align16(header + 档位边界)，向上取整到档位边界（池化与 O(1) 的代价）。 */
#define TIER_COUNT 6
#define TIER_MARK_SIZE 10240 /* 档位标记数组长度（字节，1B/槽） */
...
typedef enum {
    ...
    M_PSN_FIXED = 3,
    M_PSN_DYNAMIC = 4,
    M_PSN_TIERED = 5, /* 分档动态块（新机制，本文） */
    M_CONTIG = 6,
    M_COUNT = 7
} method_t;

static const char *method_name[] = {"fifo",
                                    "chained_hash",
                                    "balanced_tree",
                                    "psn_map_fixed",
                                    "psn_map_dynamic",
...
/* 档位判断：根据包大小用分支判断落入哪个档位（O(1)，对齐 pkt_cache.h） */
static uint8_t classify_tier(uint64_t L) {
    for (int t = 0; t < TIER_COUNT - 1; t++)
        if (L <= TIER_BOUNDARY[t])
            return (uint8_t)t;
    return (uint8_t)(TIER_COUNT - 1);
}

/* 单包档位块大小 = align16(header + 档位边界)（向上取整到该档边界） */
static uint64_t tier_block_bytes(uint64_t L) {
    uint8_t tier = classify_tier(L);
    return m_alloc(sizeof(struct mem_block_header) + TIER_BOUNDARY[tier]);
}

/* 档位机制的固定数组开销：标记数组(1B/槽) + TIER_COUNT 根环形数组(各 10240×8B) */
static uint64_t tier_ring_overhead(void) {
    return (uint64_t)TIER_MARK_SIZE * 1 +
           TIER_COUNT * (uint64_t)RING_SIZE * sizeof(void *);
}

static uint64_t bytes_psn_tiered(uint64_t N, uint64_t L) {
    /* 分档动态块（本文）：每包 [header+档位边界] + 标记数组 + 6 根环形数组 */
    return N * tier_block_bytes(L) + tier_ring_overhead();
}
...
static void run_summary(uint64_t N, const char *outdir) {
    /* 覆盖小包到 MTU，并加入 MTU 附近的细分档（1280/1400/1500），
     * 用于展示「分档动态块」在非 2 幂次包大小下的真实内部碎片。 */
    static const uint64_t L_list[] = {64,   128,  256,  512,  1024, 1280,
                                      1400, 1500, 1536, 2048, 4096};
...
static void run_multiflow(uint64_t L, const char *outdir) {
    ...
    for (int i = 0; i < nK; i++) {
        ...
        /* 分档动态块：每连接固定开销 = 标记数组 + 6 根环形数组（~490KB） */
        uint64_t ring_t = K * tier_ring_overhead();
        uint64_t block_t = total * tier_block_bytes(L);
        uint64_t alloc_t = ring_t + block_t;
        double util_t = (double)(total * L) / (double)alloc_t;
        fprintf(out,
                "vary_K_tiered,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64
                ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%.6f\n",
                K, npp, total, L, ring_t, block_t, alloc_t, util_t);
    }
    ...
    for (int i = 0; i < nn; i++) {
        ...
        /* 分档动态块：固定开销约 490KB/连接（标记数组 + 6 根环），被占用深度摊薄 */
        uint64_t ring_t = tier_ring_overhead();
        uint64_t block_t = total * tier_block_bytes(L);
        uint64_t alloc_t = ring_t + block_t;
        double util_t = (double)(total * L) / (double)alloc_t;
        fprintf(out,
                "amortize_tiered,1,%" PRIu64 ",%" PRIu64 ",%" PRIu64
                ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%.6f\n",
                npp, total, L, ring_t, block_t, alloc_t, util_t);
    }
