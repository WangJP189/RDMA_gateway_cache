...
char g_sr_target_ip[32] = {0};
int  g_sr_target_port = 0;
...
int tcp_server_setup(const char* ip, int port) {
    ...
    if (inet_pton(AF_INET, ip, &serv_addr.sin_addr) <= 0) serv_addr.sin_addr.s_addr = htonl(INADDR_ANY);
    serv_addr.sin_port = htons(port);

    if (bind(sockfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
        perror("[Dst-Server] Bind failed"); close(sockfd); return -1;
    }
    if (listen(sockfd, 5) < 0) {
        perror("[Dst-Server] Listen failed"); close(sockfd); return -1;
    }
...
int tcp_server_accept() {
    if (g_sr_server_listen_fd == -1) return -1;
    
    struct sockaddr_in client_addr;
    socklen_t client_len = sizeof(client_addr);
    
    printf("[Dst-Server] 等待连接...\n");
    int connfd = accept(g_sr_server_listen_fd, (struct sockaddr*)&client_addr, &client_len);
    if (connfd < 0) { perror("Accept failed"); return -1; }

    if (g_server_conn_fd != -1) close(g_server_conn_fd);
...
void close_sr_client_fd() {
    if (g_sr_client_fd != -1) {
        printf("[SR-Client] 关闭连接 (FD=%d)，下次将触发重连\n", g_sr_client_fd);
...
// ============================================================
void cleanup_tcp() {
    if (g_sr_server_listen_fd != -1) { close(g_sr_server_listen_fd); g_sr_server_listen_fd = -1; }
    if (g_server_conn_fd != -1)   { close(g_server_conn_fd);   g_server_conn_fd = -1; }
    if (g_sr_client_fd != -1)     { close(g_sr_client_fd);     g_sr_client_fd = -1; }
