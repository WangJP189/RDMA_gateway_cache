...
void process_rdma_packet(const unsigned char *buffer, ssize_t length) {
  ...
  if (parse_bth_header(bth_start, &opcode, &pkey, &dest_qp, &psn, &pkt_type) !=
      0) {
    printf("BTH解析失败\n");
    return;
  }
...
void process_data_packet(const unsigned char *buffer, ssize_t length,
  ...
  int udp_payload_len = ntohs(
      ((struct udphdr *)(buffer + sizeof(struct ethhdr) + ip_header_len))->len);
 ...
 */
void handle_ack_received(struct connection_bucket *bucket,
                         struct connection_key key, uint32_t epsn) {
  // 1. 入参合法性校验：哈希桶为空直接返回
  if (bucket == NULL) {
  ...
  // 2.
  // 统一处理PSN：仅保留24位有效位，过滤高位无效数据，和clean_acked_packets逻辑对齐
  ...
  while (curr != NULL) {
    ...
    if (curr_key->src_ip == key.src_ip && curr_key->dst_ip == key.dst_ip &&
        curr_key->src_qp == key.src_qp && curr_key->dst_qp == key.dst_qp &&
      ...
      printf("[INFO] handle_ack_received: 匹配到目标连接，开始清理ACK确认报文 "
             "| ACK MSN=0x%06X\n",
             ack_msn);
  ...
  if (find_conn_flag == 0) {
    printf(
        "[DEBUG] handle_ack_received: "
        "哈希桶中未匹配到指定连接[src_ip=0x%08X,dst_ip=0x%08X], 跳过ACK清理\n",
        key.src_ip, key.dst_ip);
  }
...
void handle_nack_received(struct connection_bucket *bucket,
                          struct connection_key key, uint32_t epsn) {
