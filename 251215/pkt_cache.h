...
#define BUFFER_CAPACITY (BATCH_THRESHOLD * 2 + 1)  // 阈值*2+1
#define BATCH_TIMEOUT_MS 10        // 批量处理超时(ms)
#define INIT_MEM_SIZE 64         // 初始内存块数量
#define EXTEND_MEM_SIZE 1024       // 内存扩展块数量
...
struct batch_buffer {
    ...
    int head;                     // 头指针(不再使用，保留兼容)
    int tail;                     // 尾指针(不再使用，保留兼容)
    int count;                    // 当前数量
    struct timeval last_flush;    // 上次刷新时间
    uint32_t base_psn;            // 基准PSN(用于地址映射)
    uint32_t min_psn;             // 缓冲区最小PSN(新增)
    uint32_t max_psn;             // 缓冲区最大PSN(新增)
    pthread_mutex_t lock;         // 缓冲区锁
};

// 内存块元数据结构
// struct memory_block_meta {
//     int is_free;               // 是否空闲 (1:空闲, 0:使用中，2：完成重传待释放)
//     int data_len;              // 有效数据长度
//     size_t block_size;         // 内存块总大小
//     unsigned char *block_addr; // 内存块起始地址
//     uint32_t psn;              // 关联的PSN (便于快速查找)
// };

// 线性内存区
struct linear_memory {
    struct cached_packet *blocks;       // 内存块数组
    struct memory_block_meta *meta;     // 内存块元数据数组
    size_t capacity;                    // 总容量
    size_t used;                        // 已使用数量
    struct linear_memory *next;         // 下一段内存
...
struct connection_cache {
    ...
    int running;                   // 运行标志
    struct linear_memory *first_mem; // 首个内存区
    struct linear_memory *last_mem;  // 最后一个内存区
};
