...
// 计算「单个内存块」的分配字节数（论文核心机制：按包大小动态调节）
//
//   固定模式（未定义 DYNAMIC_BLOCK）：每个包都返回 MEM_BLOCK_SIZE，小包浪费大。
//   动态模式（定义 DYNAMIC_BLOCK）  ：[mem_block_header + data_len] 向上对齐到
//                                     BLOCK_ALIGN(16B)，把内部碎片压到对齐粒度。
...
// 动态模式下单个内存块可容纳的最大数据长度（固定模式下即 MEM_BLOCK_SIZE 扣掉头）
static int max_data_len_for_block(void) {
#ifdef DYNAMIC_BLOCK
    // 动态模式：上限放宽到可容纳 RoCEv2 大包 / 巨帧（MTU 9KB 附近）。
    return 65536 - (int)sizeof(struct mem_block_header);
#else
    return MEM_BLOCK_SIZE - (int)sizeof(struct mem_block_header);
#endif
}
...
int cache_rdma_packet(struct connection_cache_array *conn, uint32_t psn,
    ...
    // 论文核心机制：按包大小动态分配内存块（而非固定 5KB）
    size_t alloc_size = mem_block_alloc_size(data_len);
    unsigned char *mem_block = (unsigned char *)malloc(alloc_size);
    if (!mem_block) {
        printf("[ERROR] 缓存数据包失败：内存块分配失败（%zu 字节）\n",
               alloc_size);
...
void age_expired_packets(struct connection_cache_array *conn) {
    ...
    if (pkt_header->psn != start_psn) {
        ...
        if (conn->ring_buf[ring_idx] != 0) {
            // 释放被覆盖的无效内存块，解决内存泄漏
            free((void *)(uintptr_t)conn->ring_buf[ring_idx]);
            // printf("[RELEASE] 已释放索引=%u 对应的失效内存块\n", ring_idx);

            // 清空环形缓冲区指针，清除脏数据
            conn->ring_buf[ring_idx] = 0;
        }

        // 重新查找有效最小PSN，更新缓存参数
        conn->start_psn = find_valid_min_psn(conn);
        // printf("[RESET] 已重置start_psn为最新有效最小值: 0x%06X\n",
        //        conn->start_psn);
        return;
    }
