...
//
// 两种可切换的分配策略，用于空间开销对比：
//   * 固定模式（现状）：每个包固定 malloc(MEM_BLOCK_SIZE) 字节，小包浪费严重。
//   * 动态模式（新机制）：按数据包大小分配 [mem_block_header + 对齐后的数据]，
//     把每个包的内部碎片降到对齐粒度（16 字节，与 glibc malloc 一致）。
//
// 通过「是否定义 DYNAMIC_BLOCK」切换；默认开启动态模式。
#define DYNAMIC_BLOCK 1

// 动态块的对齐粒度（字节）：与 glibc malloc 返回地址的 16 字节对齐一致。
// block_size = align_up(sizeof(mem_block_header) + data_len, BLOCK_ALIGN)
