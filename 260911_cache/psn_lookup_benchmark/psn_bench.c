...
int main(int argc, char **argv) {
    ...
    if (!sweep && !strcmp(mode, "lookup")) {
        ...
        uint32_t R = M / B; /* 批数 = CDF 样本数 */
        if (R == 0)
            R = 1;
        uint32_t total = R * B; /* 实际总查找次数 */
        ...
        printf("\n参数: N=%u, 总查找=%u, 批大小 B=%u, 批数 R=%u, 模式=%s, "
               "种子=0x%08X\n",
               N, total, B, R, mode, seed);
        ...
        for (method_t m = 0; m < M_COUNT; m++) {
            for (int pi = 0; pi < 2; pi++) {
                ...
                gen_psns(psns, total, 0, N, sequential);
                ...
                latency_stats s = stats_from_sorted(cycles, R);
    ...
    /* =================== 占用扫描（复杂度曲线） =================== */
        ...
        fprintf(sc, "method,N,median_ns\n");

        printf("\n[scaling] 总查找=%u, 批大小 B=%u, 批数 R=%u\n", total, B, R);
        printf("%-14s", "N");
        for (method_t m = 0; m < M_COUNT; m++)
        ...
        for (int k = 0; k < n_count; k++) {
            ...
            for (method_t m = 0; m < M_COUNT; m++) {
                ...
                bench_lookup(&c, psns, R, B, cycles);
                qsort(cycles, R, sizeof(uint64_t), cmp_u64);
                double med = (double)cycles[R / 2] / cycles_per_ns;
                printf("%14.1f", med);
                fprintf(sc, "%s,%u,%.2f\n", method_name[m], nn, med);
                cache_destroy(&c);
            }
            printf("\n");
            free(pkts);
