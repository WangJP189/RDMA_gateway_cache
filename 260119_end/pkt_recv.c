...
// ZPY
// 全局重传线程控制变量
// static volatile sig_atomic_t g_retransmit_running = 0;//重传线程运行标志
// static pthread_t g_retransmit_thread = 0; // 全局重传线程ID
// static int g_retransmit_sockfd = -1;      // 全局AF_PACKET传输的socket_fd
// static int g_tcp_server_sockfd = -1;      // 全局服务端TCP监听socket_fd
// static int g_sr_client_sockfd = -1;       // 全局客户端socket_fd
// char g_sr_target_ip[32] = {0};            // 全局SR服务端目标IP
// int  g_sr_target_port = 0;                // 全局SR服务端目标端口
// extern char g_dst_gbn_interface_name[IFNAMSIZ]; // 全局目的网关GBN重传接口名称
// extern char g_src_nack_interface_name[IFNAMSIZ]; // 全局源网关发送NACK接口名称
// extern char g_src_sr_interface_name[IFNAMSIZ];   // 全局源网关发送SR数据接口名称
extern volatile sig_atomic_t g_retransmit_running;//重传线程运行标志
...
void process_rdma_packet(const unsigned char *buffer, ssize_t length)
{
    ...
    case PKT_TYPE_ACK:
        ...
        // ZPY
        // 添加入参gateway_role
        process_ack_packet(buffer, length, bth_offset, bucket, key, psn, flow->role);
        break;
    case PKT_TYPE_NACK:
...
void process_ack_packet(const unsigned char *buffer, ssize_t length, int bth_offset,
                        struct connection_bucket *bucket, struct connection_key key, uint32_t epsn, enum gateway_role role)
{
                         ...
                         struct connection_bucket *bucket, struct connection_key key, uint32_t epsn, enum gateway_role role)
{
    ...
    struct connection_key reverse_key = create_connection_key(key.dst_ip, key.src_ip,
                                                              ...
                                                              key.pkey);
    // 获取反向Bucket
    struct connection_bucket *reverse_bucket = get_connection_bucket(reverse_key);
    if (reverse_bucket == NULL)
    {
        printf("[ERROR] handle_ack_received: 反向哈希桶指针为空，无法处理ACK\n");
        return;
    }
    ...
    if (target_cache != NULL)
    {
        ...
        {
            printf("[WARN] handle_ack_received: 匹配到连接[src_ip=0x%08X,dst_ip=0x%08X], 但缓存环形数组未初始化，跳过ACK清理\n",
                   key.src_ip, key.dst_ip);
            pthread_rwlock_unlock(&reverse_bucket->rwlock); // 提前解锁，避免泄漏
            return;
        }
...
// ZPY
//  计算丢包位图
lost_segment *calculate_bitmap_loss(struct connection_cache_array *conn, uint32_t *seg_num)
{
    ...
    lost_segment *lost_segs = (lost_segment *)malloc(MAX_LOST_SEGMENTS * sizeof(lost_segment));
    seg_count = 0;
    // 遍历所有PSN
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (is_packet_lost(conn, current_psn))
        {
            ...
            {
                // 当seg_count == MAX_LOST_SEGMENTS时，表示这是第MAX_LOST_SEGMENTS+1个丢包段，超出限制，不做记录，退出循环
...
int clean_nacked_packets(struct connection_cache_array *conn, uint32_t nack_psn)
{
    if (!conn)
    {
    ...
    uint32_t temp_end = (nack_psn - 1) & PSN_MASK; // NACK确认的最后PSN是nack_psn-1
    uint32_t current_start = conn->start_psn;
    uint32_t current_end = conn->end_psn;
    int cleaned_count = 0;

    printf("[NACK CLEAN] 开始清理已确认报文：NACK PSN=%u | 原始PSN范围=[%u~%u] | 清理区间=[%u~%u]\n",
    ...
    // nak流程epsn不会等于end_psn，因此直接更新start_psn就可以
...
void set_sr_client_target_info(const char* ip, int port) {
    ...
    printf("[SR-Config] 对端地址已更新: %s:%d\n", g_sr_target_ip, g_sr_target_port);
}
...
// ZPY
int get_or_create_g_retransmit_sockfd()
{
...
void retransmit_rdma_packet(const unsigned char *packet_data, int length,
                            const char *interface_name, uint32_t psn, uint32_t dest_qp)
{
    // TODO: 实现RDMA数据包的重传逻辑
    ...
    memcpy(ifname, interface_name, IFNAMSIZ);
    ...
    int psock = get_or_create_g_retransmit_sockfd();
    if (psock < 0)
    {
    ...
    ssize_t sent = sendto(psock, frame, length, 0, (struct sockaddr *)&sll, sizeof(sll));
    if (sent < 0)
    {
    ...
    {
        printf("通过 %s 发送 %zd 字节 (PSN=%u, QP=%u)\n", ifname, sent, psn, dest_qp);
    }
    // psock不关闭，保持全局socket复用
    free(frame);
}

void set_gateway_msg(struct connection_key key, struct connection_cache_array *conn,lost_segment *ls_sgments,
                     gateway_control_msg *gw_control_msg, gateway_data_msg *gw_data_msg, uint32_t seg_num, size_t msg_size)
{
    ...
    {
        gw_data_msg->lost_segments[i].start_psn = htonl(ls_sgments[i].start_psn);
...
{
    // TODO: 实现发送SR重传请求给源网关的逻辑
    int sockfd = get_sr_client_fd();
    if (sockfd == -1)
    {
    ...
    gateway_control_msg *gw_control_msg = (gateway_control_msg *)malloc(msg_size);
    ...
    // 填充控制信息结构体数据
    set_gateway_msg(key, conn, ls_sgments, gw_control_msg, gw_data_msg, seg_num, msg_size);
    ...
    {
        printf("丢包段 %d: start_psn=%d, num=%d\n", i, gw_data_msg->lost_segments[i].start_psn, gw_data_msg->lost_segments[i].loss_num);
    }
...
void retransmit_latest_one_packet(struct connection_cache_array *conn, char *interface_name, uint32_t epsn, uint32_t src_qp)
{
    ...
    {
        uint32_t psn = (lost_psn + 1 + i) & PSN_MASK; // epsn对应的数据包丢失，从epsn+1开始查找该丢包段后面的第1个缓存报文
        // 重传最近一个丢包段后面的第1个缓存报文
        if (!is_packet_lost(conn, psn))
        {
            // 触发重传该数据包
            int ring_index = psn % RING_BUFFER_SIZE;
            unsigned char *packet_data = (unsigned char *)conn->ring_buf[ring_index];
            struct mem_block_header *header = (struct mem_block_header *)packet_data;
            unsigned char *rdma_packet_data = packet_data + sizeof(struct mem_block_header);
            // 发送重传数据包
            // 此处的dest_qp应为NACK报文的发送端QP（src_qp）
            retransmit_rdma_packet(rdma_packet_data, header->data_len, interface_name, header->psn, src_qp);
            printf("[GBN_RETRANSMIT] 重传PSN=%u | 环形数组索引=%d | 内存块地址=0x%lx\n",
...
// ZPY
// GBN重传函数
int dst_gateway_gbn_retransmit(struct connection_cache_array *conn, char *interface_name, uint32_t nack_epsn, uint32_t src_qp)
{
    // GBN重传：重传从nack_epsn开始到最近一个丢包段后面的第1个缓存报文，触发目的主机快速发送新的NAK
    // 维护一个epsn，记录最近一个丢包段的位置
    char dst_interface_name[IFNAMSIZ];
    memcpy(dst_interface_name, interface_name, IFNAMSIZ);
    ...
    for (uint32_t i = 0; i < nack_end_count; i++)
    {
        ...
        {
            // 发现丢包段，停止重传
            epsn = psn; // 更新epsn为丢包段的起始PSN, 该psn对应的数据包丢失，从psn+1开始查找该丢包段后面的第1个缓存报文
            loss_packet_found = 1;
            break;
        }
        else
...
// ZPY
// 处理收到的网关消息
void handle_gateway_msg(gateway_control_msg *gw_control_msg, gateway_data_msg *gw_data_msg, const char *interface_name)
{
    // 转换字节序
    ...
    struct connection_bucket *bucket = get_connection_bucket(key);
    // 获取连接缓存
    struct connection_cache_array *conn = get_connection_cache_array(bucket, key);

    // 检查重传类型：如果设置为GBN重传，则丢弃SR重传请求
    if (conn->retransmit_type == RETRANSMIT_GBN)
    {
        printf("[GBN_RETRANS] 重传类型为GBN，丢弃收到的SR重传请求\n");
    ...
    for (uint32_t i = 0; i < seg_num; i++)
    {
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            ...
            if (!is_packet_lost(conn, psn))
            {
                struct mem_block_header *packet_data = (struct mem_block_header *)conn->ring_buf[ring_index];
                struct mem_block_header *header = (struct mem_block_header *)packet_data;
                unsigned char *rdma_packet_data = (unsigned char *)packet_data + sizeof(struct mem_block_header);
                if (header->psn == psn)
                {
                    retransmit_rdma_packet(rdma_packet_data, header->data_len, interface_name, psn, gw_control_msg->dest_qp);
                    printf("[SR_RETRANS] 重传PSN=%u, QP=%u\n", psn, gw_control_msg->dest_qp);
                }
            }
        }
    }
...
void handle_sr_requests()
{
    ...
    // 根据实际情况设置接口名称
    char src_sr_interface_name[IFNAMSIZ];
    memcpy(src_sr_interface_name, g_src_sr_interface_name, IFNAMSIZ);
    int client_fd;
    struct sockaddr_in client_addr;
    socklen_t client_len = sizeof(client_addr);
    size_t buffer_size = sizeof(uint32_t) * 2 + MAX_LOST_SEGMENTS * sizeof(lost_segment);
    gateway_control_msg *gw_control_msg = (gateway_control_msg *)malloc(sizeof(gateway_control_msg));
    ...
    // 可以修改全局变量中断监听循环
    while (g_retransmit_running)
    {
        // 接受客户端连接
        client_fd = accept(g_tcp_server_sockfd, (struct sockaddr *)&client_addr, &client_len);
        if (client_fd == -1)
        {
            perror("accept失败");
            continue; // 继续监听下一个连接
        ...
        while (g_retransmit_running)
        {
            ...
            // 基本长度校验：至少要包含控制头和数据头
            if (total_received < (ssize_t)(sizeof(gateway_control_msg) + sizeof(gateway_data_msg)))
            {
                printf("接收数据长度不足: %ld < %zu, 丢弃本次消息\n",
                       total_received, sizeof(gateway_control_msg) + sizeof(gateway_data_msg));
                // 跳过本次循环，继续等待下一条消息
                continue;
            }
            // 解析gateway_data_msg
            ...
            // 重传类型为SR时才处理重传请求
            handle_gateway_msg(gw_control_msg, gw_data_msg, src_sr_interface_name);
             // 检查是否收到辅助数据
...
void *sr_retransmit_worker(void *arg)
{
    ...
    // 创建socket
    if ((g_tcp_server_sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
    {
        perror("socket创建失败");
        exit(EXIT_FAILURE);
    }
    ...
    // 设置SO_REUSEADDR选项
    int opt = 1;
    if (setsockopt(g_tcp_server_sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1)
    {
        perror("setsockopt失败");
    ...
    // // 这里port使用的全局变量g_sr_target_port
    // server_addr.sin_port = htons(g_sr_target_port);

    if (bind(g_tcp_server_sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1)
    {
        perror("bind失败");
        close(g_tcp_server_sockfd);
        return NULL;
    ...
    // 监听
    if (listen(g_tcp_server_sockfd, 5) == -1)
    {
        perror("listen失败");
        close(g_tcp_server_sockfd);
        return NULL;
...
int start_retransmit_thread(int port)
{
    ...
    *port_ptr = port;
    ...
    if (g_retransmit_running)
    {
        printf("[SR_THREAD] SR重传线程已在运行\n");
        free(port_ptr);
        return -1;
    }
    g_retransmit_running = 1;

    if (pthread_create(&g_retransmit_thread, NULL, sr_retransmit_worker, (void *)port_ptr) != 0)
    {
        perror("创建SR重传线程失败");
        g_retransmit_running = 0;
        free(port_ptr);
        return -1;
    }
...
int stop_retransmit_thread()
{
    ...
    // 2) 关闭监听套接字，唤醒阻塞的 accept
    if (g_tcp_server_sockfd != -1)
    {
        close(g_tcp_server_sockfd);
        g_tcp_server_sockfd = -1;
    }

    // 3) 关闭可能存在的客户端/发送套接字，防止阻塞的 recvmsg
    if (g_sr_client_sockfd != -1)
    {
        close(g_sr_client_sockfd);
        g_sr_client_sockfd = -1;
...
void send_nak_to_source_host(const unsigned char *pack_data, int length,
                             ...
                             uint32_t nack_epsn, uint32_t dest_qp)
{
    // TODO: 实现向源主机发送NAK的逻辑
    // 修改源MAC发送给源主机, 调用retransmit_rdma_packet函数
    printf("[NAK_TO_HOST] 向源主机发送NAK: epsn=%u \n", nack_epsn);
    retransmit_rdma_packet(pack_data, length, interface_name, nack_epsn, dest_qp);
}
                          ...
                          struct connection_bucket *bucket, struct connection_key key, uint32_t epsn, enum gateway_role role)
{
    ...
    struct connection_bucket *reverse_bucket = get_connection_bucket(reverse_key);
     // 获取网关角色
    enum gateway_role reverse_role = (role == src_gateway) ? dst_gateway : src_gateway;
    uint32_t src_qp = key.src_qp;
    if (reverse_bucket == NULL)
    {
    ...
    if (cache_array != NULL)
    {
        ...
        printf("[INFO] handle_ack_received: 匹配到目标连接，开始清理ACK确认报文 | NACK PSN=0x%06X\n", nack_epsn);
        ...
        default:
            ...
            {
                printf("[ERROR] handle_nack_received: NACK清理失败，错误码=%d\n", clean_ret);
            }
            break;
        }
    }
    ...
    {
        // 未匹配到对应连接的日志打印
        printf("[DEBUG] handle_nack_received: 哈希桶中未匹配到指定连接[src_ip=0x%08X,dst_ip=0x%08X], 跳过NACK清理\n",
               key.src_ip, key.dst_ip);
    }
    ...
    {
        printf("[NACK] DESTINATION_GATEWAY收到NACK: src_ip=%s, dst_ip=%s, dest_qp=%u, nack_epsn=%u\n",
               key.src_ip, key.dst_ip, key.dst_qp, nack_epsn);
        // interface_name应为目的网关对应的接口
        // 根据实际情况设置接口名称
        char dst_gbn_interface_name[IFNAMSIZ];
        memcpy(dst_gbn_interface_name, g_dst_gbn_interface_name, IFNAMSIZ);
        ...
        if (is_packet_lost(cache_array, nack_epsn))
        {
            ...
            printf("[SR_REQUEST] 目的网关发起SR重传请求给源网关: src_ip=%s, dst_ip=%s, qp=%u\n",
        ...
        else
        {
            ...
            printf("[GBN_RETRANSMIT] 目的网关进行GBN重传: src_ip=%s, dst_ip=%s, qp=%u\n",
                   key.src_ip, key.dst_ip, key.dst_qp);
            int retransmit_count = dst_gateway_gbn_retransmit(cache_array, dst_gbn_interface_name, nack_epsn, key.src_qp);
            if (retransmit_count == 0)
            {
                printf("[ERROR] 目的网关GBN重传数据包失败或者没有数据包需要重传\n");
            }
            else
            {
                printf("[GBN_RETRANSMIT] 目的网关GBN重传完成，重传报文数量=%d\n", retransmit_count);
            }
        }
    }
    else
    { // 源网关
        printf("[NACK] SOURCE_GATEWAY收到NACK: src_ip=%s, dst_ip=%s, dest_qp=%u, nack_epsn=%u\n",
            key.src_ip, key.dst_ip, key.dst_qp, nack_epsn);
        ...
        // 检查重传类型：如果已设置为SR重传，则丢弃此NAK
        if (cache_array->retransmit_type == RETRANSMIT_SR)
        {
            // 前面已经释放了nack_epsn之前的缓存报文，这里直接返回
            printf("[SR_RETRANS] 重传类型为SR，丢弃收到的NAK\n");
            return;
        }
        // interface_name应为源网关对应的接口
        // 根据实际情况设置接口名称
        char src_nack_interface_name[IFNAMSIZ];
        memcpy(src_nack_interface_name, g_src_nack_interface_name, IFNAMSIZ);
        //  步骤2：检查缓存情况
        ...
        if (!is_packet_lost(cache_array, nack_epsn))
        {
            ...
            cache_array->retransmit_type = RETRANSMIT_SR;
        }
        else
        {
            // NAK报文中psn在源网关上不存在，设置重传类型为GBN重传，修改源MAC发送给源主机
            printf("[GBN_RETRANS] 源网关不存在epsn=%u的缓存，设置重传类型为GBN重传，向源主机发送NAK\n", nack_epsn);
            cache_array->retransmit_type = RETRANSMIT_GBN;
            send_nak_to_source_host(buffer, length, src_nack_interface_name, nack_epsn, key.dst_qp);
        }
    }
    // 解锁哈希桶
    pthread_rwlock_unlock(&reverse_bucket->rwlock);
