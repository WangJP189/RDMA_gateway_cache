...
char g_dst_gbn_interface_name[IFNAMSIZ] = {0}; // 全局目的网关GBN重传接口名称
char g_src_nack_interface_name[IFNAMSIZ] = {0}; // 全局源网关发送NACK接口名称
char g_src_sr_interface_name[IFNAMSIZ] = {0};   // 全局源网关发送SR数据接口名称
// ZPY
...
{
    int sockfd = get_or_create_g_retransmit_sockfd();
    if (sockfd < 0) {
        printf("创建重传socket失败\n");
    } else {
...
{
    char dst_gbn_interface[IFNAMSIZ];
    char src_nack_interface[IFNAMSIZ];
    char src_sr_interface[IFNAMSIZ];

    printf("DST GBN Interface Name: ");
    if (scanf("%15s", dst_gbn_interface) != 1) return;

    printf("SRC NACK Interface Name: ");
    if (scanf("%15s", src_nack_interface) != 1) return;

    printf("SRC SR Interface Name: ");
    if (scanf("%15s", src_sr_interface) != 1) return;

    // 复制到全局变量
    strncpy(g_dst_gbn_interface_name, dst_gbn_interface, IFNAMSIZ - 1);
    g_dst_gbn_interface_name[IFNAMSIZ - 1] = '\0';

    strncpy(g_src_nack_interface_name, src_nack_interface, IFNAMSIZ - 1);
    g_src_nack_interface_name[IFNAMSIZ - 1] = '\0';

    strncpy(g_src_sr_interface_name, src_sr_interface, IFNAMSIZ - 1);
    g_src_sr_interface_name[IFNAMSIZ - 1] = '\0';
