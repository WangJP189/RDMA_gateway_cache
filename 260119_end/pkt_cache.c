...
void age_expired_packets(struct connection_cache_array* conn)
{
    ...
    if (pkt_header->psn != start_psn) {
        printf("[WARN] 老化处理：start_psn=0x%06X 与内存块PSN=0x%06X 不一致，跳过老化\n", start_psn, pkt_header->psn);
        return;
    }
    ...
    // 满足老化条件，执行完整的数据包老化清理
    printf("[INFO] 老化处理：start_psn=0x%06X 数据包已超时（当前=%lu ms/缓存=%lu ms），开始执行数据包老化清理\n",
...
int clean_idle_entry(uint32_t bucket_idx)
{
    ...
    while (curr != NULL) {
        ...
        if ((curr_time - curr->cache_array->last_active_stamp) > CONN_IDLE_EXPIRE_THRESHOLD) {
 ...
 * @note 彻底解决原逻辑"一次性遍历所有桶耗时过长"的问题，打散遍历压力，让出CPU资源
 */
...
void *connection_aging_thread(void *arg)
{
    ...
    // 线程正常退出，打印日志
    printf("[INFO] connection aging thread exit success, total cleaned connections: %u\n", g_total_cleaned_conn);
    pthread_exit(NULL);
    return NULL;
}
...
{
    if (g_conn_buckets == NULL) {
        fprintf(stderr, "[ERROR] connection_aging_thread_start: global conn buckets not initialized\n");
        return -1;
    }
