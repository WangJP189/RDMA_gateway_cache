...
// ZPY
// 处理网关消息
void handle_gateway_msg(gateway_control_msg *gw_control_msg, gateway_data_msg *gw_data_msg, 
...
// ZPY
// 重传epsn后最新一个缓存的数据包
void retransmit_latest_one_packet(struct connection_cache_array *conn, char *interface_name, 
    uint32_t epsn, uint32_t src_qp);

// ZPY
// GBN重传
int dst_gateway_gbn_retransmit(struct connection_cache_array *conn, char* interface_name, 
    uint32_t nack_epsn, uint32_t src_qp);
