...
| 16 | SR `retrieve_set` 统一交付契约：按 PSN 升序交付（fifo/hash qsort、tree 中序零排序、dynblock 扫槽零排序）；exp1b reorder∈{0,1} 维度废止（升序上移到接口内） | 2026-09-19 第 4B「SR 统一交付契约」 | 诚实披露：SR 计时=定位+memcpy+升序交付全过程；tree/dynblock O(N) 零排序、fifo/hash O(k log k) qsort——大 N、小 k 下 hash 的 qsort 占优（见 CLAIM_GUARDRAILS） | 是（须写 retrieve_set 升序契约 + SR 各方法升序成本归属） |
