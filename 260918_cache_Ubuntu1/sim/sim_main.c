...
#undef OPEN
    ...
    fprintf(tw->f_resize,   "epoch,S_old,S_new,reason,moved_pkts,resize_ns,drain_ns\n");
...
static void write_resize_events(trace_writer_t *tw, const conn_t *gw) {
    for (uint32_t k = 0; k < gw->n_rev; k++) {
        ...
        const char *reason = (e->reason == REV_GROW)      ? "grow"
                           ...
                           :                                "overflow_grow";
        fprintf(tw->f_resize, "%u,%u,%u,%s,%u,%llu,%llu\n",
                e->epoch, e->S_old, e->S_new, reason, e->moved_pkts,
                (unsigned long long)e->resize_ns, (unsigned long long)e->drain_ns);
    }
}
...
int sim_run(const cfg_t *cfg, sim_stats_t *st, trace_writer_t *tw) {
    ...
    /* summary.csv：关键计数器落盘（供脚本提取；n_drop/n_resize 等不在既有 per-epoch trace 中） */
    {
        char sp[512];
        snprintf(sp, sizeof(sp), "%s/summary.csv", cfg->out_dir);
        FILE *sf = fopen(sp, "w");
        if (sf) {
            fprintf(sf, "n_store,n_lookup,n_resize,n_drain,n_evict,n_drop,n_ovf_ins,"
                        "n_ooo_drop,nak_n,hit,miss,bytes_retrans\n");
            fprintf(sf, "%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu\n",
                    (unsigned long long)gw.n_store, (unsigned long long)gw.n_lookup,
                    (unsigned long long)gw.n_resize, (unsigned long long)gw.n_drain,
                    (unsigned long long)gw.n_evict, (unsigned long long)gw.n_drop,
                    (unsigned long long)gw.n_ovf_ins, (unsigned long long)gw.n_ooo_drop,
                    (unsigned long long)st->nak_n, (unsigned long long)st->hit,
                    (unsigned long long)st->miss, (unsigned long long)st->bytes_retrans);
            fclose(sf);
        }
    }
