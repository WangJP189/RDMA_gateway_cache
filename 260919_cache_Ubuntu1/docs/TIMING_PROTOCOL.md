...
## 9. 复杂度口径（各方法 store / GBN / SR 的渐近复杂度，2026-09-20 复核）

> 依据：exp1a p50 主表（store）、exp1b p50 + n_cmp 表（GBN/SR）、exp2（空间）。
> 数字出处见 RESULTS.md 各节与 `out/exp1b_lookup/exp1b_analysis_data.md`；此处只登记渐近项，禁手抄测量值。

| 方法 | store | GBN retrieve_range | SR retrieve_set（升序交付） |
|---|---|---|---|
| FIFO Queue | O(1)（环尾写） | O(N)（n_cmp≈N/2 顺序扫） | O(k log k) 排序 + O(1) 查找（sorted_retrieve_set） |
| Chained Hash | O(1)（链头插） | O(1)（n_cmp=1.0） | O(k log k) 排序 + O(1) 查找（sorted_retrieve_set） |
| Balanced Tree | O(log N)（AVL 插） | O(log N)（n_cmp≈log N） | O(k log k) 排序 + k×O(log N) 查找（tree_search_set，主）；中序 O(N)（xval） |
| PSN Mapping（adaptive S） | O(1) Φ（psn%N 槽写） | O(1) Φ（n_cmp=0 位置映射） | **O(n + span/64) 位图**（conn_retransmit_set，零排序）；span 超限退化 O(N) |
| index-only（Φ） | O(1) Φ（纯算术） | O(1) Φ（n_cmp=0） | O(k log k) 排序 + O(1) 查找（sorted_retrieve_set） |

- **SR 升序交付契约**（#16/#18）：fifo/hash/index_only 无天然 PSN 序 ⇒ 显式 `sort_u32_asc`（O(k log k)）后逐包检索；tree 主路径同构（排序 + k 次 O(log N) 查找）；dynblock 位置映射扫槽零排序。
- **位图快路径**（#17）：`conn_retransmit_set` O(n + span/64)，栈上 1KB 位图、无 malloc、无比较排序；退化条件（span>8192 / 24-bit 跨界 / span>N）落 `conn_retransmit_set_ref` O(N)。
- **主战场划分**：PSN 主战场是 GBN 区间提取 O(1)（n_cmp=0）；SR 升序交付非 PSN 独有优势（大 N、小 k 下 hash 的 qsort 亦占优），但位图快路径使 PSN 在 SR 下仍优于三基线（见 fig_pareto）。
