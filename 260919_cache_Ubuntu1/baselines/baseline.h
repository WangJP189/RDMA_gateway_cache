...
struct b_cache {
    ...
    /* ---- 第 4B/P1：SR 交付口径（由 bench 从 cfg.e1b_sort_impl / e1b_tree_inorder 透传） ---- */
    int         sort_impl;    /* 排序原语：0=qsort（主）1=内联插入排序（xval）；作用于 fifo/hash/index_only/tree */
    int         tree_inorder; /* tree SR 路径：0=tree_search_set（排序+k 查找，主）1=中序 O(N)（xval） */
};
...
/* exp2_tail 观测：读 dynblock 内部计数（溢出插入 / 丢弃 / 实际切换次数）。
 * 非 dynblock 一律置 0。机制不改，仅把 conn_t 观测值透出给实验脚本。 */
