...
/* ---- SR 位图快路径（第 4B/P0：把 conn_retransmit_set 的 O(N) 全环扫描换成 O(n + span/64)） ----
 * CFG_SR_BM_MAX_N：请求 PSN 跨度 span=hi-lo+1 的上限；超过则退化回 conn_retransmit_set_ref
 *   （O(N) 参考路径，正确性优先）。位图存栈上（bm[CFG_SR_BM_WORDS]），无 malloc。
 * 数学前提：span ≤ CFG_SR_BM_MAX_N 且 (lo&0xFFFFFF)+span ≤ 2^24（不跨界）时，
 *   d=psn-lo 单调增 ⇔ PSN 升序 ⇔ 槽下标 (phi(lo)+d)%N 单调，故位图扫槽天然升序、零比较排序。 */
#define CFG_SR_BM_MAX_N   8192u
#define CFG_SR_BM_WORDS   ((CFG_SR_BM_MAX_N + 63u) / 64u)
...
#define CFG_E1B_SORT_IMPL       0u      /* SR 升序交付的排序原语（第 4B/P1）：0=glibc qsort（主）1=内联插入排序（xval）。
                                         * 作用于 fifo/hash/index_only 的 sorted_retrieve_set 与 tree 的 tree_search_set（排序+k 次查找）。 */
#define CFG_E1B_TREE_INORDER    0u      /* tree SR 路径（第 4B/P1）：0=tree_search_set（排序+k 次 O(log N) 查找，主）
                                         * 1=中序遍历 O(N)（xval 交叉验证）。保留中序仅作对照：证明 O(k log N) 优于中序 O(N)。 */
...
#define CFG_E2_FIXED_L       4096u    /* ring_fixed 固定槽大小 S（=最大 MTU，代表「不做弹性的固定块」） */
#define CFG_E2_TAIL_FRAC_LIST  { 0.0, 0.01, 0.03, 0.06, 0.10, 0.20, 0.30 } /* 尾包占比 f：包尺寸 (1-f)@MTU + f@U[1,MTU]；exp2_tail 扫描点 */
#define CFG_E2_TAIL_FRAC_N     7u
...
typedef struct {
    ...
    uint32_t e1b_sort_impl;
    uint32_t e1b_tree_inorder;
    /* ---- 实验二 ---- */
    ...
    /* ---- 实验二尾部敏感性（追加在末尾：out_dir 后天然 8 字节对齐，避免动中间布局） ---- */
    uint32_t e2_tail_frac_n;
    uint32_t _pad3;              /* 显式对齐：e2 double 数组前补足 8 字节 */
    double   e2_tail_frac_list[CFG_E2_TAIL_FRAC_N];
} cfg_t;
