 ...
 *
 * 方法（6 个，全部有界预分配，无界版每 store malloc 不进 exp2）：
 ...
 *   psn_dynblock（弹性）   —— S=ceil_class(pl)=pl（5 档 MTU 精确命中），环池 stride=align16(S)（24B 头已移除）
 *   ring_fixed             —— S=e2_fixed_l=4096 固定（「不做弹性的固定块」对照；与 exp1a/exp1b 同名同义）
 *
 ...
 *   ② ring_fixed 在 256B 档崩到 ~6%（4128B 槽只装 256B），证明「弹性槽大小」解决了
 *      固定块空间利用率低下的问题；
 *   ③ 弹性 S 收敛到各 MTU 档（block_S 列），且槽浪费 = align16 填充 + 12B meta。
 *
...
int main(int argc, char **argv) {
    ...
    fprintf(f, "# methods: fifo_bounded / chained_hash_bounded / balanced_tree_bounded / psn_dynblock(elastic) / ring_fixed(S=%u)\n",
            cfg.e2_fixed_l);
    fprintf(f, "# psn_dynblock elastic S=ceil_class(pl)=pl (5 MTU tiers exact); ring_fixed S=%u; block_S=0 for non-dynblock\n",
            cfg.e2_fixed_l);
    fprintf(f, "# allocated_bytes = cache_footprint_bytes (pool + index/buckets + meta, sizeof measured); utilization is fraction 0..1\n");
    fprintf(f, "# overhead_per_pkt (B) = (allocated_bytes - payload_bytes) / N：每驻留包分摊的空间开销\n");
    fprintf(f, "# lower_bound_util = pl / (align16(pl) + sizeof(slot_meta_t))：位置槽理想利用率上界（仅对齐填充+12B meta，无索引/桶/FIFO 指针），方法无关\n");
    fprintf(f, "method,payload,N,block_S,payload_bytes,allocated_bytes,utilization,overhead_per_pkt,lower_bound_util\n");
    ...
    for (uint32_t pi = 0; pi < cfg.e2_pktsize_n; pi++) {
        ...
        for (uint32_t mi = 0; mi < N_METHODS; mi++) {
            ...
            double   ovh_per_pkt = (alloc >= pbytes) ? (double)(alloc - pbytes) / (double)N : 0.0;
            double   lower_util  = (double)pl / (double)(align16(pl) + (uint32_t)sizeof(slot_meta_t));

            fprintf(f, "%s,%u,%u,%u,%llu,%llu,%.6f,%.3f,%.6f\n",
                    m->name, pl, N, block_S,
                    (unsigned long long)pbytes, (unsigned long long)alloc, util,
                    ovh_per_pkt, lower_util);
            printf("  %-22s pl=%4u S=%4u  payload=%10llu B  alloc=%10llu B  util=%6.2f%%  ovh=%.1f B\n",
                   m->name, pl, block_S,
