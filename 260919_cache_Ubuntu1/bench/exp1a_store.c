 ...
 *   balanced_tree_bounded / psn_dynblock / ring_fixed / index_only。
 ...
 *   ring_fixed         —— 全程 adaptive_enable=0，S=4096 固定（「不做自适应」的对照；与 exp1b 同名同义）。
...
static const method_t METHODS_POOLED[] = {
    ...
    { "ring_fixed",            mk_dyn,    0, 1 },   /* S=4096 固定（去弹性；与 exp1b 同名同义） */
...
int main(int argc, char **argv) {
    ...
    fprintf(f, "# dynblock: psn_dynblock=收敛后冻结(预热 adaptive=1, 记录收敛 S), ring_fixed=S=4096 固定(去弹性)\n");
