...
/* 分档环形数组：档位边界为幂次 2 {256,512,1024,2048,4096}；
...
static uint64_t bytes_psn_tiered(uint64_t N, uint64_t L) {
    /* 分档动态块（本文）：每包 [header+档位边界] + 标记数组 + 5 根环形数组 */
...
static void run_summary(uint64_t N, const char *outdir) {
    ...
    static const uint64_t L_list[] = {64,   128,  256,  512,  1024,
                                      1280, 1400, 1500, 2048, 4096};
...
static void run_multiflow(uint64_t L, const char *outdir) {
    ...
    for (int i = 0; i < nK; i++) {
        ...
        /* 分档动态块：每连接固定开销 = 标记数组 + 5 根环形数组（~410KB） */
    ...
    for (int i = 0; i < nn; i++) {
        ...
        /* 分档动态块：固定开销约 410KB/连接（标记数组 + 5 根环），被占用深度摊薄 */
