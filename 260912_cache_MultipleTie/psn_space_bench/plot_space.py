...
METHODS = [
    ...
    ("psn_map_dynamic", "PSN Map (dynamic)",   "#1f77b4", "-"),  # 精确动态块
    ("psn_map_tiered",  "PSN Map (tiered)",    "#17becf", "-"),  # 分档动态块（本文）
    ("contiguous",      "Contiguous (ideal)",  "#7f7f7f", "--"),  # 理想下界
]
LINE_WIDTH = {"psn_map_dynamic": 2.8, "psn_map_tiered": 2.8, "contiguous": 2.2}
MARKERS = {"fifo": "s", "chained_hash": "^", "balanced_tree": "o",
           "psn_map_fixed": "v", "psn_map_dynamic": "D",
           "psn_map_tiered": "P", "contiguous": "*"}
...
def plot_multiflow(dirpath, outdir):
    ...
    with open(path) as f:
        ...
        for line in f:
            ...
            elif scenario == "amortize_tiered":
                amort_tiered["npp"].append(int(cols[2]))
                amort_tiered["util"].append(float(cols[8]) * 100.0)
    ...
    if amort_tiered["npp"]:
        ot = np.argsort(amort_tiered["npp"])
        npp_t = np.array(amort_tiered["npp"])[ot]
        util_t = np.array(amort_tiered["util"])[ot]
        ax.plot(npp_t, util_t, label="PSN Map (tiered)", color="#17becf",
                marker="P", lw=2.8, markersize=5)
    ax.set_xscale("log")
