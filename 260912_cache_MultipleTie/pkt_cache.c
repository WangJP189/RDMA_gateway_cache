...
// ==================== 档位机制全局变量（论文核心机制） ====================
// 仿真 MTU（字节）：模拟建联协商结果，默认 4096（RoCEv2 常见 MTU）。
// 档位边界由 g_mtu 生成 {mtu/16, mtu/8, mtu/4, mtu/2, mtu}（幂次 2，便于分支分类）。
uint32_t g_mtu = 4096;
uint32_t g_tier_boundary[TIER_COUNT] = {256, 512, 1024, 2048, 4096};

// 初始化档位边界：根据协商 MTU 生成幂次 2 的档位边界
void init_tier_boundaries(uint32_t mtu) {
    if (mtu == 0)
        mtu = 4096;
    g_mtu = mtu;
    for (int t = 0; t < TIER_COUNT; t++) {
        uint32_t boundary = mtu >> (TIER_COUNT - 1 - t); // mtu/16, mtu/8, ...
        // 最小档位边界保底 64B（避免极小 MTU 导致最小档为 0）
        if (boundary < 64)
            boundary = 64;
        g_tier_boundary[t] = boundary;
    }
}
...
// 档位空闲池：从该档的空闲链表取一块（无则 malloc），复用释放的内存块
static void *pool_alloc(struct connection_cache_array *cache, uint8_t tier) {
    struct mem_block_pool *p = &cache->pool[tier];
    if (p->free_head) {
        void *block = p->free_head;
        p->free_head = *(void **)block; // 取下链表头
        p->reuse_count++;
        return block;
    }
    p->alloc_count++;
...
// 档位空闲池：把内存块回收到该档的空闲链表（不真正 free，留给下次复用）
static void pool_free(struct connection_cache_array *cache, uint8_t tier,
                      void *block) {
    struct mem_block_pool *p = &cache->pool[tier];
    *(void **)block = p->free_head;
    p->free_head = block;
...
static struct connection_cache_array *alloc_cache_array(int size) {
    ...
    // 1. 分配档位标记数组（1 字节/槽，索引 = psn % TIER_MARK_SIZE）
    //    用 0xFF 填充（TIER_NONE，表示"无缓存"），calloc 只能清零，需手动填。
    cache->tier_mark = (uint8_t *)malloc(TIER_MARK_SIZE * sizeof(uint8_t));
    if (!cache->tier_mark) {
        perror("[ERROR] tier_mark 内存分配失败");
        free(cache);
        return NULL;
    }
    memset(cache->tier_mark, TIER_NONE, TIER_MARK_SIZE);

    // 2. 为每个档位分配一根环形数组（存内存块地址，calloc 清零为 NULL）
    for (int t = 0; t < TIER_COUNT; t++) {
        cache->tier_ring[t] = (uintptr_t *)calloc(size, sizeof(uintptr_t));
        if (!cache->tier_ring[t]) {
            perror("[ERROR] tier_ring 内存分配失败");
            // 回滚：释放已分配的档位环与标记数组
            for (int j = 0; j < t; j++)
                free(cache->tier_ring[j]);
            free(cache->tier_mark);
            free(cache);
            return NULL;
        }
        cache->tier_ring_len[t] = size;
        // 3. 初始化该档空闲池（块大小 = header + 档位边界，对齐 16B）
        cache->pool[t].free_head = NULL;
        cache->pool[t].block_size = tier_block_size((uint8_t)t);
        cache->pool[t].alloc_count = 0;
        cache->pool[t].reuse_count = 0;
    }
...
void free_cache_array(struct connection_cache_array *cache) {
    ...
    // 1. 释放各档位环形数组里残留的数据包（直接 free，不做池回收）
    for (int t = 0; t < TIER_COUNT; t++) {
        if (cache->tier_ring[t]) {
            for (uint32_t i = 0; i < cache->tier_ring_len[t]; i++) {
                if (cache->tier_ring[t][i] != 0) {
                    free((void *)(uintptr_t)cache->tier_ring[t][i]);
                    cache->tier_ring[t][i] = 0;
                }
            }
            free(cache->tier_ring[t]);
            cache->tier_ring[t] = NULL;
        }
    }

    // 2. 释放各档空闲池里残留的内存块（回收后未被复用的，避免泄漏）
    for (int t = 0; t < TIER_COUNT; t++) {
        void *b = cache->pool[t].free_head;
        while (b) {
            void *next = *(void **)b;
            free(b);
            b = next;
...
void release_cached_block(struct connection_cache_array *conn, uint32_t psn) {
    ...
    unsigned char *block = take_cached_block(conn, psn, &tier);
    if (block != NULL)
        pool_free(conn, tier, block);
}
...
int is_psn_expired(struct connection_cache_array *conn, uint32_t psn,
                   uint64_t current_ts) {
    // O(1) 取块：psn -> 档位标记 -> 分档环（无遍历）
    unsigned char *mem_block = get_cached_block(conn, psn);
...
int batch_clean_psn_range(struct connection_cache_array *conn, uint32_t start,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        // O(1) 取并摘除：读取档位号、清空标记与槽位
        uint8_t tier = TIER_NONE;
        unsigned char *block = take_cached_block(conn, current_psn, &tier);

        // 空指针跳过（已清理/无数据）
        if (block == NULL)
            continue;

        // 回收到该档位空闲池（不真正 free，留给下次复用）
        pool_free(conn, tier, block);
        cleaned_count++;
...
uint32_t find_valid_min_psn(struct connection_cache_array *conn) {
    ...
    // 遍历档位标记数组：每个标记槽 i 对应 psn%TIER_MARK_SIZE == i；
    // 该槽位若为非空档，则从对应分档环的槽位 i 读块（环长 == 标记数组长度）。
    for (uint32_t i = 0; i < TIER_MARK_SIZE; i++) {
        uint8_t tier = conn->tier_mark[i];
        if (tier >= TIER_COUNT)
            continue; // 空档位跳过

        unsigned char *block =
            (unsigned char *)(uintptr_t)conn->tier_ring[tier][i];
        if (!block)
            continue;

        struct mem_block_header *hdr = (struct mem_block_header *)block;
...
// 单个内存块可容纳的最大数据长度 = 最大档位边界（协商 MTU）。
// 超过 MTU 的包无法被任何档位容纳（仿真中 MTU 由建联协商决定）。
static int max_data_len_for_block(void) {
    return (int)g_tier_boundary[TIER_COUNT - 1];
}
...
int add_to_connection_cache(struct connection_cache_array *conn_cache,
    ...
    int max_allowed_len = max_data_len_for_block();
    ...
    // 校验4：连接缓存结构体为空
    if (conn_cache->tier_mark == NULL || conn_cache->array_length == 0) {
        printf("[ERROR] 数据包缓存失败：conn_cache->tier_mark无效\n");
        return RETRANS_INVALID_PARAM;
    }
...
// 缓存RDMA数据包到内存，并将地址存入「分档环形数组 + 档位标记数组」
int cache_rdma_packet(struct connection_cache_array *conn, uint32_t psn,
                      const unsigned char *data, int data_len) {

    // 论文核心机制①：档位判断（分支判断 O(1)），确定该包落入哪个档位
    uint8_t tier = classify_tier((uint32_t)data_len);

    // 论文核心机制②：从该档位空闲池取内存块（无则 malloc，复用释放块）
    unsigned char *mem_block = (unsigned char *)pool_alloc(conn, tier);
    if (!mem_block) {
        printf("[ERROR] 缓存数据包失败：档位%u内存块分配失败（%zu 字节）\n",
               tier, conn->pool[tier].block_size);
    ...
    // 论文核心机制③：O(1) 取并摘除旧块（读档位标记 -> 清标记 -> 清分档环槽）
    uint8_t old_tier = TIER_NONE;
    unsigned char *old_mem_block = take_cached_block(conn, psn, &old_tier);
    if (old_mem_block != NULL) {
        // 回收到旧块所在档位的空闲池（而非简单 free）
        pool_free(conn, old_tier, old_mem_block);
        cache_count--;
    ...
    // 论文核心机制④：O(1) 存块（写分档环槽 + 写档位标记）
    set_cached_block(conn, psn, tier, mem_block);
    printf("[CACHE] 数据包PSN=%u → 档位=%u | 环形数组索引=%d | "
           "内存块首地址=0x%lx | 数据长度=%d | 时间戳=%lu ms\n",
           psn, tier, ring_index, (uintptr_t)mem_block, data_len,
           header->recv_stamp);
    ...
    if (conn->start_psn == PSN_INVALID) {
        ...
        // 刚存好 psn，若它比当前 start_psn 更小，则推进 start_psn
...
int clean_acked_packets(struct connection_cache_array *conn, uint32_t ack_msn) {
    ...
    if (!psn_in_ring_range(temp_end, current_start, current_end)) {
        // ack_msn不在当前缓存的PSN范围内
        // 若为非回绕区间且ack_msn > end_psn，仅清理到end_psn为止
        if (psn_greater_than(temp_end, current_end)) {
            temp_end = current_end;
            // printf("[ACK CLEAN] ACK MSN=%u 超出当前缓存PSN范围[%u~%u]，"
            //        "仅清理到end_psn=%u\n",
...
void age_expired_packets(struct connection_cache_array *conn) {
    // 第一步：基础空指针校验,确保conn和tier_mark有效
    if (conn == NULL) {
        printf("[ERROR] 老化处理：conn缓存结构体为空\n");
        return;
    }
    if (conn->tier_mark == NULL) {
        printf("[ERROR] 老化处理：conn->tier_mark为空（缓存初始化失败）\n");
        return;
    }
    ...
    // 校验1+2：start_psn 对应位置无数据（标记空或槽空），说明 start_psn 失效 ->
    // 重新查找有效最小PSN
    uint32_t start_psn = conn->start_psn;
    unsigned char *mem_block = get_cached_block(conn, start_psn);
    if (mem_block == NULL) {
        conn->start_psn = find_valid_min_psn(conn);
        return;
    }
    struct mem_block_header *pkt_header = (struct mem_block_header *)mem_block;

    // 校验3：start_psn与内存块PSN不匹配，发生覆盖 → 重新查找有效最小PSN
    if (pkt_header->psn != start_psn) {
        // 摘除被覆盖的无效内存块并回收到空闲池，解决内存泄漏
        uint8_t dirty_tier = TIER_NONE;
        unsigned char *dirty_block =
            take_cached_block(conn, start_psn, &dirty_tier);
        if (dirty_block != NULL)
            pool_free(conn, dirty_tier, dirty_block);
...
void print_all_connections_status() {
    ...
    for (uint32_t bucket_idx = 0; bucket_idx < TABLE_SIZE; bucket_idx++) {
        // 加桶级读锁，避免遍历过程中链表被修改
