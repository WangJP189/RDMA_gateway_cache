...
void *timer_epoll_thread_func(void *arg) {
    ...
    while (!gd->stop) {
        int nfds = epoll_wait(gd->timer_epoll_fd, events, MAX_EPOLL_EVENTS,
                              100); // 等待定时器事件，超时时间100ms
        if (nfds < 0) {
            if (errno == EINTR) {
                continue; // 被信号中断，继续循环
            }
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            // 读取定时器到期事件，清除定时到期状态，避免再次epoll_wait到
            uint64_t cnt;
            ssize_t s = read(task_info->timer_fd, &cnt, sizeof(cnt));
            // timerfd 是 Linux 内核提供的一种特殊文件描述符：
            // 1、当定时器到期时，文件描述符变为可读
            // 2、读取的数据固定是 8 字节（uint64_t）
            // 3、要么读取成功（返回 8），要么读取失败（返回 1）
            if (s == sizeof(cnt)) {
                event_count++;
...
static void process_delayed_rnr_retry(struct global_data *gd,
    ...
    found_entry->rnr_timer_fd = -1; // 清理entry中的定时器fd

    // 只有目的网关会创建rnr延时定时器
    // 源网关收到nak rnr报文时做如下处理，不会创建rnr延时定时器
    // 01.未缓存nak_psn报文，直接构造nak rnr报文发送给源主机
    // 02.缓存nak_psn报文，直接返回，等待sr重传请求
    if (found_entry->role == DST_GATEWAY) {
        dst_gateway_nak_v4(gd, found_entry, found_entry->rnr_retry_psn);
    }
...
static void process_delayed_nak_retry(struct global_data *gd,
    ...
    if (found_entry->nak_timer_fd != task_info->timer_fd) {
        // 定时器到期从epoll中取出写入任务队列但还未调度，此时再次收到nak此时会先释放老的定时器fd，
        // 再创建新的定时器fd，之后旧的定时器又调度，当前调度定时器fd和entry中定时器fd不相同，
        // 因再次收到nak流程已经释放了旧的定时器fd，所以旧定时器调度时直接返回即可，不用再重复释放旧定时器fd等操作。
        printf("  Timer fd mismatch (entry_fd=%d, task_fd=%d), ignoring "
               "delayed NAK retry\n",
               found_entry->timer_fd, task_info->timer_fd);
        pthread_spin_unlock(&found_entry->lock);
        pthread_spin_unlock(&gd->cache_tbl_v4->bucket_locks[bucket_idx]);
        return;
    }
    found_entry->nak_timer_fd = -1; // 清理entry中的定时器fd

    // 只有数据报文方向的目的网关才会触发延时NAK处理
    send_sr_request_v4(gd, found_entry, task_info->nak_psn);
...
int create_nak_delay_timer(struct global_data *gd,
                           ...
                           uint32_t nak_psn) {
    // 如果已有定时器，先清理
    if (entry->nak_timer_fd >= 0) {
        epoll_ctl(gd->timer_epoll_fd, EPOLL_CTL_DEL, entry->nak_timer_fd, NULL);
        close(entry->nak_timer_fd);
    ...
    if (timer_fd >= 0) {
        printf("NAK delay retry scheduled: delay=%ums, PSN=0x%06X\n",
               DELAY_NAK_INTERVAL, nak_psn);
        return ERR;
    }

    // 保存到entry中
    entry->nak_timer_fd = timer_fd;
    entry->nak_psn = nak_psn;
    return OK;
}

void update_nak_delay_timer(struct cache_entry_v4 *entry) {
    // 只有目的网关才会发送SR重传请求
    if (entry->role != DST_GATEWAY || entry->nak_timer_fd <= 0) {
        return;
    }
    modify_timer_time(entry->nak_timer_fd, DELAY_NAK_INTERVAL);
}
...
// 清理定时器
void cleanup_entry_timer(struct cache_entry_v4 *entry, struct global_data *gd) {
    if (entry->rnr_timer_fd >= 0) {
        // 从epoll中移除定时器
        epoll_ctl(gd->timer_epoll_fd, EPOLL_CTL_DEL, entry->rnr_timer_fd, NULL);
        ...
        entry->rnr_timer_fd = -1;
        entry->rnr_retry_count = 0;
        printf("Aging thread: cleaned rnr timer for entry\n");
    }
    ...
    if (entry->nak_timer_fd >= 0) {
        ...
        // 标记为无效，由定时器系统清理
        entry->nak_timer_fd = -1;
        printf("Aging thread: cleaned delay_nak timer for entry\n");
    }
}
