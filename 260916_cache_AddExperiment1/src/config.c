...
typedef enum { T_U32, T_U64, T_DBL, T_STR, T_U32A, T_PHASE } ftype_t;
...
static int apply_override(cfg_t *c, const field_t *f, const char *val) {
    ...
    case T_PHASE: {
        cfg_phase_t ph[32]; uint32_t n = 0;
        if (parse_phase_list(val, ph, 32, &n)) return -1;
...
static void cfg_print_usage(const char *prog) {
    ...
    printf("    --e3-preset=default|mixed     --e3-phases=a,b;c,d;...\n");
    printf("    --out=<dir>                   输出目录\n");
}
